...
def get_product_queryset(request):
    ...
    if search_query:
        # Search by name, brand, short description, long description
    ...
    # Sorting
    sort_by = request.GET.get('sort_by', 'default') # 'default', 'price_asc', 'price_desc', 'rating_desc', 'name_asc'
    if sort_by == 'price_asc':
        queryset = queryset.order_by('price')
    elif sort_by == 'price_desc':
        queryset = queryset.order_by('-price')
    elif sort_by == 'rating_desc':
        queryset = queryset.order_by('-rating', '-review_count') # Sort by rating, then popularity
    elif sort_by == 'name_asc':
        queryset = queryset.order_by('name')
    else: # default - by created_at (newest first)
...
def product_list(request):
    ...
    paginator = Paginator(initial_products, 8) # Show 8 products per page initially
    ...
    context = {
        ...
        'pick_up_line': random.choice(PICK_UP_LINES), # Random pick-up line
    }
...
def api_product_list(request):
    ...
    paginator = Paginator(products_qs, 8) # 8 products per page for API too
    page_number = request.GET.get('page')

    try:
        page_obj = paginator.page(page_number)
    except Exception: # Handle PageNotAnInteger, EmptyPage
...
def product_detail(request, pk):
    ...
    ).exclude(pk=pk).order_by('?')[:4] # Get 4 random related products
